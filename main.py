...
def add_event():
    try:
        ...
        if chek1 == chek2 and datetime.datetime.strptime(data["endTime"], '%H:%M') <= datetime.datetime.strptime(data["startTime"], '%H:%M'):
            return Response(status=405, response='Invalid input.')
        if not exists:
            return Response(status=405, response='Invalid input.')

        new_event = Event(creator_userId=data['creator_userId'], title=data['title'], aboutEvent=data['aboutEvent'],
...
def get_participants_event(eventId, userId):
    try:
        ...
        if len(output) == 0:
            return Response(status=404, response='User is not participant in event')
...
def update_participants_event(eventId, userId):
    try:
        ...
        if not db_event_participant or db_event_participant.user_userId != userId:
            return Response(status=400, response='Invalid ID supplied.')
        data = request.get_json()
        try:
        ...
        #    db_event_participant.user_status = data["user_status"]
            session.execute(update(Participant).where(Participant.c.event_eventId == eventId, Participant.c.user_userId == userId).values(user_status=data["user_status"]))
        # Save changes
        session.commit()
...
def delete_participants_event(eventId, userId):
    try:
        ...
        # Delete
        deleting = delete(Participant).where(Participant.c.event_eventId == eventId, Participant.c.user_userId == userId)
        session.execute(deleting)
        session.commit()
...
def testing():
    user1 = User(userId=3, username="annli_345", firstName="Ann", lastName="Li", email="annli_345@email.com", password="AnnieIsTheBest24243")
    #user2 = User(userId=2, username="robjames11223", firstName="Robert", lastName="James", email="robjames11223@email.com", password="R0b3rtJ4m3S1", phone="697-345-3425")
    #event1 = Event(eventId=1, creator_userId=1, title="Ann's Marriage", startDate='2023-01-03', endDate='2023-01-05', startTime='09:30')
